...
int main(int argc, char *argv[]){
    ...
    int in_fd[num_procs][2];
    int out_fd[num_procs][2];
    ...
    for (int i = 0; i < num_procs; i++){
        ...
        if (pid == -1){
            ...
            child_handler(training, testing, K, fptr, in_fd[i][0], out_fd[i][1]); 
    ...
    // the number of test images to process to their write pip

    int N = testing->num_items / num_procs + ((testing->num_items % num_procs) != 0);

    for (int i = 0; i < num_procs; i++){
        int start_idx = i * N;

        if (write(in_fd[i][1], &start_idx, sizeof(int)) == -1){
            perror("write to pipe");
            exit(1);
        }

        if (i == num_procs - 1 && testing->num_items % N != 0){
            N = testing->num_items - i * N;
        }

        if (write(in_fd[i][1], &N, sizeof(int)) == -1){ 
            perror("read to pipe");
            exit(1);
        }
    ...
    int read_child;
    for (int i = 0; i < num_procs; i++){
        if (read(out_fd[i][0], &read_child, sizeof(int)) == -1){
            perror("read to pipe");
        }
        total_correct += read_child;
        close(out_fd[i][0]);
    }
